...
import { upsertBusiness } from "../lib/business-upsert";
  ...
  input_schema: {
    ...
    properties: {
      ...
      source_urls: {
        type: "array",
        items: { type: "string" },
        description: "Every URL actually used as evidence",
      },
      homepage_headline: { type: "string" },
      main_cta: { type: "string", description: "e.g. call | book | quote form | none visible" },
      seo_score: { type: "integer", minimum: 1, maximum: 5 },
      conversion_score: { type: "integer", minimum: 1, maximum: 5 },
...
export const auditBusiness = task({
  id: "audit-business",
  run: async (payload: {
    accountId: number;
    auditId: number;
    ...
    const audit = (await db
      .prepare("SELECT account_id, category, location FROM vis_audits WHERE id = ?")
      .get(payload.auditId)) as { account_id: number; category: string; location: string } | undefined;
    if (!audit) throw new Error(`audit ${payload.auditId} not found`);
    // Defense in depth: this task is only ever fanned out from run-audit, which
    // already validated the job's account_id, but a bad payload should never be
    // able to write a business row into another tenant's audit.
    if (Number(audit.account_id) !== Number(payload.accountId)) {
      throw new Error(`audit ${payload.auditId} does not belong to account ${payload.accountId}`);
    }
    ...
    const searchCallCount = researchResponse.content.filter(
    ...
    const { id, created } = await upsertBusiness(payload.auditId, meta, db);
